...
const scheduleMatchDayNotifications = async () => {
  try {
    ...
    const notifications = schedule
      ...
      .map(s => ({
        id: s.id,
        title: 'Time to Pray for the Nations ⚽',
        body: s.nations
...
const RAW_COUNTRIES = [
  ...
  },
  {
    ...
    m: "Qatar's population is over 85% migrant workers — Nepalis, Filipinos, Indians, and Pakistanis who built the tournament stadiums and now fill the city of Doha. Many come from Hindu, Buddhist, and Muslim backgrounds with little gospel access at home. Qatar is a temporary gathering of nations: workers who carry what they encounter back to their home countries.",
  ...
  },
  {
    ...
    m: "Curaçao makes a rare tournament appearance for this small Caribbean island — a Dutch territory with a warm, syncretic Christian heritage. The island has seen a wave of Venezuelan and Haitian migrants in recent years, many in precarious conditions. Curaçao's diaspora in the Netherlands represents a bridge community with gospel potential in both directions.",
  ...
  },
  {
    ...
    m: "Cabo Verde makes its first-ever tournament appearance — a tiny island nation of 560,000 with a diaspora larger than its home population scattered across Portugal, the Netherlands, and New England. The diaspora carries gospel potential: Cape Verdean believers in Boston and Lisbon are uniquely positioned to reach their own communities and beyond.",
...
/* ─── SCHEDULE DATA ─── */
const RAW_SCHEDULE = [
  { d:"Jun 11", full:"Thursday, June 11", img:"/images/day-01.png", feat:["Mexico","South Africa"], dev:`The 2026 tournament is one of the greatest celebrations of the nations, thanks to the global sport of soccer. This year, more nations than ever will participate as the field of teams expands to 48, and for the first time, the tournament will be held in 3 host nations (U.S., Mexico, & Canada). Truly, the eyes of the world will be focused on soccer over the next month, and some have estimated that the 104 matches will bring between 1.5 and 2.5 million international visitors to North America. This means the story of this year's tournament is the story of the nations coming to us, and that is a story that God set in motion from the beginning. Read Acts 17:26-27.`, pray:`Prayer for the nations to know and experience God through this year's tournament. • Pray that people will see and experience God's heart for the nations, and many will come to know him as Lord and Savior. • Pray for the nations represented in this year's tournament who are considered unreached by the Gospel. Ask the Lord to bring them into contact with believers, so they can hear the Good News.`, matches:[{t:"3:00 PM",a:"Mexico",b:"South Africa",g:"A",v:"Mexico City Stadium"},{t:"10:00 PM",a:"South Korea",b:"Czechia",g:"A",v:"Guadalajara Stadium"}] },
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        <div style={{ background: C.white, borderRadius: 16, padding: 18, marginBottom: 14, border: `1px solid ${C.blue}30`, boxShadow: "0 4px 20px rgba(27,69,106,0.12)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            {userProfile?.uid && (
              <button
                onClick={async () => {
                  const text = 'I just prayed for the nations today using Pray for the Nations 🌍⚽ Join me at prayforthecup.com';
                  try {
        ...
        {/* Other Nations to Pray For Today */}
        {(() => {
          const featSet = new Set(day.feat || []);
          const matchNations = (matchesToShow || []).flatMap(m => [m.a, m.b]).filter(n => !featSet.has(n));
          const unique = [...new Set(matchNations)];
          ...
          return (
            <div style={{ marginBottom: 14 }}>
              ...
              {unique.map(name => {
                const nation = RAW_COUNTRIES.find(c => c.n === name);
                if (!nation) return null;
                return (
                  <button key={name} onClick={() => setSelectedNation(nation)} style={{
                    display: "flex", alignItems: "center", gap: 14, width: "100%",
                    background: C.white, border: `2px solid ${C.blue}30`,
                    borderRadius: 14, padding: "14px 16px", cursor: "pointer",
                    textAlign: "left", marginBottom: 10, boxShadow: "0 4px 20px rgba(27,69,106,0.12)",
                  }}>
                    <FlagImg iso={nation.iso} f={nation.f} size={38} />
...
const TeamsTab = ({ gameState, updateGameState, userProfile, autoJoinCode, onAutoJoinConsumed, tabFocusVersion }) => {
  ...
  const handleInvite = (team) => {
    ...
    const shareText = `Join my prayer team "${team.name}" on Pray for the Nations — praying through all 48 nations of the 2026 tournament. Join here:`;

    if (navigator.share) {
      navigator.share({
        title: 'Join my team on Pray for the Nations',
        text: shareText,
        url: inviteUrl,
  ...
  async function handleNudge(targetUid, memberName) {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Pray for the Nations', text: selectedMessage, url: 'https://prayforthecup.com/app' });
      } else {
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  const text = `Join my prayer team on Pray for the Nations! Code: ${teamShareBanner.teamCode} — prayforthecup.com/app?join=${teamShareBanner.teamCode}`;
    ...
    platformPreview = (
      <div style={{ background: "#fff", borderRadius: 14, padding: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.15)", marginBottom: 16 }}>
        <div style={{ fontFamily: "-apple-system, 'Helvetica Neue', sans-serif", fontWeight: 600, fontSize: 14, color: "#000", textAlign: "center", marginBottom: 6 }}>
          "Pray for the Nations" Would Like to Send You Notifications
        </div>
    ...
    platformPreview = (
      <div style={{ background: "#fff", borderRadius: 24, padding: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.15)", marginBottom: 16 }}>
        ...
        <div style={{ fontFamily: "Roboto, sans-serif", fontSize: 13, color: "#49454F", marginBottom: 20, lineHeight: 1.5 }}>
          Pray for the Nations wants to send you daily prayer reminders.
        </div>
    ...
    platformPreview = (
      <div style={{
        ...
        <div style={{ flex: 1, fontFamily: "sans-serif", fontSize: 13, color: "#202124", lineHeight: 1.4 }}>
          <strong>Pray for the Nations</strong> wants to send you notifications
        </div>
          ...
          >
            ...
            >
              ...
              <div style={{ fontFamily: "Montserrat, sans-serif", fontSize: 11, color: C.blue, textAlign: "center", marginTop: 20 }}>
                Pray for the Nations · prayforthecup.com
              </div>
          ...
          >
            ...
            >
              ...
              <div style={{ marginBottom: 28 }}>
                <div style={{ fontFamily: "Montserrat, sans-serif", fontSize: 14, fontWeight: 700, color: "#00BAF8", marginBottom: 8 }}>Pray for the Nations</div>
              ...
              <div style={{ fontFamily: "Montserrat, sans-serif", fontSize: 11, color: "#8ADBFF", textAlign: "center", marginTop: 16 }}>
                Pray for the Nations · v1.0 · 2026
              </div>
